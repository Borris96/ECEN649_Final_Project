...
for faceNum = 1:faceSize
    ...
    img = int2str(faceNum);
    fullPath = strcat(str,prefix,img,'.png');
    img = imread(fullPath);
    % convert to integral image
...
for nonFaceNum = 1:nonFaceSize
    % read non-face image
    str = 'dataset/trainset/non-faces/1 (';
...
window = 19;
% store min errors and corresponding inedxes of top haar features
haarMinError = zeros(size(haars,1), 1);
haarMinIndexes = zeros(size(haars,1), 4);
haarFalseCount = zeros(size(haars, 1), 2);
% Record selected features for each haar type
features = zeros(size(haars,1), 1);
faceRatings = zeros(size(haars,1), 1);
nonFaceRatings = zeros(size(haars,1), 1);
% number of training iterations
for iterations = 1:5
    out = strcat('***** Iteration #',int2str(iterations),'\n');
    fprintf(out);
    % initialize classifier container
    weakClassifiers = {};
    % iterate through features
    for haar = 1:5
        minError = 1; % Minimum Error in each haar type when running adaboost
        minHaarX = -1; minHaarY = -1; minPixelX = -1; minPixelY = -1;
        minIndexes = [minHaarX, minHaarY, minPixelX, minPixelY];
                    ...
                    for haarY = dimY:dimY:window-pixelY
                        ...
                        for iter = 1:50
                            ...
                            if faceRating < 0.05 % if less than 5% faces misclassified
                                ...
                                if totalError < .5 % if less than 50% total error
                        ...
                        if size(storeRatingDiff) > 0
                            ...
                            [imgWeights,alpha, minError, minHaarX, minHaarY, minPixelX, minPixelY, falsepos, falseneg] = adaboost(thisClassifier,allImages,imgWeights, minError, minHaarX, minHaarY, minPixelX, minPixelY);
                            minIndexes = [minHaarX, minHaarY, minPixelX, minPixelY];
                            minFalses = [falsepos, falseneg];
        ...
        % features(haar) = maxRatingDiff;
        % fprintf('Total Error \n');
        % display(totalError);
        printout = strcat('Finished Haar #',int2str(haar),'\n');
        fprintf(printout);
        haarMinError(haar) = minError;
        haarMinIndexes(haar, : ) = minIndexes;
        haarFalseCount(haar, : ) = minFalses;
    end 
end
...
classifierSize = 286;
selectedClassifiers = zeros(classifierSize,12);
for i = 1:classifierSize
